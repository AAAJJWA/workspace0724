 ...
 * 
 * 전역 스코프 : 코드 어디서든 접근 가능한 영역(전역에서 선언된 변수/함수)
 * 함수 스코프 : 함수 내부에서만 접근 가능한 영역(var 키워드로 선언한 값)
 * 블록 스코프 : {} 블록 내부에서만 접근이 가능한 영역(let, const 키워드로 선언한 값)
...
// var는 함수 스코프이기때문에 for 문내에 새로운 i가 생기지 않고 전역에 i를 저장함
// for 문이 종료되어도 전역 i는 그대로 유지됨
// let, const는 블록 스코프이기 때문에 for 문내에 새로운 스코프를 만들고 j를 해당 스코프에 저장해 사용한 뒤,
// for 문이 종료되면 
