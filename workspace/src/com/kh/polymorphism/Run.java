...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * 		(부모 클래스가 위치할 수 있음) = (자식 클래스가 위치 할 수 있음) -> 업 캐스팅
		 * 		실제로 어디까지 접근이 가능한가 = 실제로 어떤 형태의 메모리가 생성되는가
		 * 
		 * */
//		
//		Cake c1 = new CheeseCake();
//		// Cake까지 접근가능하며 메모리 공간은 실제 CheesCake의 공간까지 존재함
//		c1.sweet();
//		// 오버라이딩시에는 실제 메모리에 있는 재정의된 메서드가 호출됨
//		c1.yummy();
//		// 다운 캐스팅 : 자식의 참조변수로 타입을 변경(업 캐스팅 관계에 있을 경우에만 가능)
//		((CheeseCake)c1).milky();
		
		// CheeseCake c2 = new Cake();
		// 실제 메모리가 Cake 이므로 CheeseCake 공간에 접근할 수 없음
		// -> 접근 범위가 실제 메모리의 크기보다 클 수 없음
		
		// 1. 부모타입의 래퍼런스(참조변수)로 부모객체를 다루는 경우
		Car c1 = new Car("빨간색", "가솔린", 20201);
		c1.drive();
		...
		c2.moveAvante();
		// 부모참조변수로 업 캐스팅 가능 -> 오버라이딩된 메서드는 생성 메모리를 기준으로 호출됨
		((Car)c2).drive();
//		((Car)c2).moveAvante();	// 업 캐스팅시 자식요소의 메모리 접근이 불가
		 ...
		 * 2. DownCastion
		 * 	  부모타입 -> 자식타입으로 형변환
		 * 	  강제 형변환 해야됨, 업캐스팅 관계에 있을 때만 가능
		 * 	  ex) ((Sonata)c).moveSonata();
		 * 	  		((자식)부모).자식메서드();
		 * */
		
		c2.driveCar(c1); // c1 -> 실제 메모리 Car
		c2.driveCar(c2); // c2 -> 실제 메모리 Avante
		c2.driveCar(c3); // c3 -> 실제 메모리 Sonata
