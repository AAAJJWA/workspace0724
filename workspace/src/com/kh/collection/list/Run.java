...
public class Run {
	...
	public static void main(String[] args) {
		...
		// List(순서O, 중복 허용)
		// ArrayList(배열기반), LinkedList(노드 기반), Vector, Stack(Vector 기반, LIFO구조)
		
		java.util.List<E> list = new ArrayList();	// 크기를 지정하지 않아도 됨
		...
		list.add(new Human("", 25));
		list.add(new Human("이무진", 25));
		
		System.out.println(list);
		
		// 지정된 크기보다 더 많이 추가해도 에러가 발생하지 않음. 	장점 1. 크기 제약X
		// 다양한 타입의 데이터를 담을 수 있음 			   	장점 2. 여러타입 보관 가능
		...
		// 4. remove(E e) : 객체를 직접 지정해서 삭제(사용을 위해서는 객체의 equals 를 적절하게 오버라이딩 해줘야 함
		
		// 5. set(int index, E e) : 해당 인덱스의 값을 전달받은 e 객체로 덮어 씌움
