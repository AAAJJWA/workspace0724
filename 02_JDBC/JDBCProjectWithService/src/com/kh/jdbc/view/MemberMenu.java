...
public class MemberMenu {
	...
	public void bulkInsertMembers() {
			System.out.println("===== 회원 몇 명 추가 =====");
			
			System.out.print("추가 할 인원 : ");
	...
	public void bulkDeleteMembers() {	// 아이디를 ,로 구분해서 입력받아서
		
		System.out.println("========= 회원 탈퇴 ===========");
		System.out.print("탈퇴할 회원들의 아이디(, 로 구분) : ");
		String userId = sc.nextLine();
		
		String[] userIds = userId.split(",");
		for (int i = 0; i < userIds.length; i++) {
			userIds[i] = userIds[i].trim();  // 양쪽 공백 제거
		}
		
		mc.bulkDeleteMember(userIds);
