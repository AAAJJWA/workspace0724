...
public class ServerStream {
	 ...
	 * 	   FileWriter : 파일을 직접적으로 연결해서 2byte 단위로 출력할 수 있는 기반 스트림
	 * 	   BufferedWriter : 편의성과 속도향상에 도움을 주는 보조 스트림(기반 스트림 없이 사용 불가)
	 * */
...
//			
//			// 2. 보조 스트림 생성시 기반 스트림 객체를 전달하여 생성 
...
//			fw.close()
	...
	public void fileSaveWithStream() {
		...
		try {
			...
			// 2. 기반 스트림 -> OutputStream 을 Writer 로 사용할 수 있게 보조
			OutputStreamWriter osw = new OutputStreamWriter(fo, "UTF-8");
			
			// 3. 보조 스트림 생성시 기반 스트림 객체를 전달하여 생성 
	...
	public void fileRead() {
		// FileReader : 파일을 직접적으로 연결해서 2byte 단위로 입력받을 수 있는 기반 스트림
...
//			System.out.println(br.readLine());
//			System.out.println(br.readLine());
//			System.out.println(br.readLine());	// 파일에 모든 내용을 읽어온 후에는 null 이 반환됨
	...
	/*
	 * 직렬화
	 * 객체는 메모리안ㅇ에서 존재하는 인스턴스 개념이기 때문에 그대로 전송할 수 없음, 직렬화 후 이진데이터로 외부에 전송 가능
	 * - 객체(Object)를 byte 형태로 변환하는 과정
	 * - 파일에 저장하거나 네트워크로 전송할 수 있도록 함
	 * - 자바에서는 클래스를 직렬화하기 위해 직렬화 대상클래스에게는 implements Serializable 해야 함
	 * 
	 * 역직렬화
	...
	// 파일 -> 프로그램(입력) : 객체바이트코드(역직렬화)
		public void ObjectRead() {
			...
			try(ObjectInputStream ois = new ObjectInputStream(new FileInputStream("product1.dat"));) {
				Product p1 = ois.readObject();	// Object
			} catch (FileNotFoundException e) {
				System.out.println("에러 : 객체 읽기 중 I/O 에러 발생");
				e.printStackTrace();
