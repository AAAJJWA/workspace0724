...
public class StringAPI {
	...
	public void method01() {
		...
		// String 클래스에 toString 메서드는 이미 오버라이딩 되어 있음
		// 그래서 참조변수 출력시 Object 의 toString 형식(타입@주소값16진수)이 아니라 String 의 저장된 문자열이 반환됨
		
		System.out.println(str1 == str2);
		// String 객체의 주소값을 비교하기 때문에 false 가 나옴
		
		System.out.println(str1.equals(str2));
		// String 클래스에서 equals 메서드도 이미 오버라이딩 되어있음(주소값 비교가 아니라 문자열 값을 비교하도록)
		
		System.out.println(str1.hashCode());
		System.out.println(str2.hashCode()); 
		// String 클래스에서 hashCode 메서드도 이미 오버라이딩 되어있음(주소값이 아닌 문자열을 가지고 해시값을 만들도록)
		
		// System.identityHashCode(참조변수)
		// 해당 참조변수의 메모리값을 기반으로 한 해시값을 반환
		...
		// 상수이기 때문에 새로운 값을 String pool 에 만들고 참조하도록 되어 있음
	}
	...
	public void method02() {
		...
		// 문자열에서 전달받은 index 위치에 문자를 추출
		char ch = str1.charAt(6);
		System.out.println("ch : " + ch);
		
		// 문자열.contains(찾고자하는 문자열) : boolean
		// 문자열에 전달된 문자열이 포함되어 있는지 확인하여 결과를 반환
		System.out.println("str1에 ello 라는 문자열이 포함 ? : " + str1.contains("ello"));
		System.out.println("str1에 bye 라는 문자열이 포함 ? : " + str1.contains("bye"));
	...
	public void method03() {
		...
		System.out.println();
		// 분리된 문자열 배열을 다시 String 으로 연결하는 방법
		// String.join(구분자, 배열);
	    ...
	    // System.out.println(stn.nextToken()); 더이상 토큰이 남아있지 않을 때 다음 토큰 호출시 예외발생
