...
public class MemberController {
	...
	public void bulkDeleteMember(String[] userIds) {
		
		for(int i = 0; i < userIds.length; i++) {
			Member m = new Member();
			m.setUserId(userIds[i]);
			
			int result = ms.bulkDeleteMember(m);
			if(result > 0) {
				new MemberMenu().displaySuccess("성공적으로 회원탈퇴가 되었습니다.");
			} else {
				new MemberMenu().displayFail("회원탈퇴에 실패하였습니다.");
...
//	public void bulkInsertMembers(String userId, String userPwd, String userName, String gender, String age, String email,
//			String phone, String address, String hobby, int count) {
//		
//		//view로부터 전달받은 값을 바로 dao에 전달x
//		//vo에 잘 담아서 전달
//		Member m = new Member(userId, userPwd, userName, gender, 
