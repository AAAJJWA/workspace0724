...
public class Run {
	 ...
	 * 
	 * 추상메서드만선언 가능(구현부가 없는 메서드)
	 * 	- java8 이후 default 메서드, static 메서드도 사용 가능
	 * 모든 메서드는 묵시적으로 public abstract 키워드가 생성(명시적으로 사용 가능)
	 * 모든 변수는 묵시적으로 public static final(정적 상수, 명시적 사용 가능)
	 * 인터페이스를 implements 하는 클래스는 반드시 인터페이스의 모든 추상메서드를 구현
